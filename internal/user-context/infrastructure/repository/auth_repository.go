...
func NewAuthRepository(dm *db.DBManager) AuthRepository {
	return &authRepository{dm: dm}
}
...
func (r *authRepository) GetUserByProviderID(ctx context.Context, provider domain.AuthProvider, providerID string) (*domain.User, error) {
	dbConn := r.dm.DB(ctx)
	var user domain.User
