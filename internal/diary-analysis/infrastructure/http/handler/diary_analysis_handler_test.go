...
func (m *MockDiaryAnalysisUsecase) GetAccuracyScoreByDate(ctx context.Context, userID uuid.UUID, dateStr string) (map[string]interface{}, error) {
	args := m.Called(ctx, userID, dateStr)
	return args.Get(0).(map[string]interface{}), args.Error(1)
}
...
// GetWeekAccuracyScore with valid user ID and date - success
func TestDiaryAnalysisHandler_GetWeekAccuracyScore_Success(t *testing.T) {
	t.Parallel()
	...
	userID := uuid.New()
	date := "2026-01-20"
	expectedScore := map[string]interface{}{
		"2026-01-20": 50,
		"2026-01-21": 70,
	}
